...
def window_based(model, num_prune, calibration_dataset):
    ...
    intermediate_outputs = compute_intermediate_outputs(
        model, calibration_dataset, last_token=True
    )
    ...
    for layer1 in tqdm(range(num_blocks), desc="Layer similarity"):
        ...
        sims = [
            torch.nn.functional.cosine_similarity(
                intermediate_outputs[ci][layer1],
                intermediate_outputs[ci][layer2],
                dim=0,
            ).item()
    ...
    for ci in range(len(intermediate_outputs)):
        for li in range(1, num_blocks):
            ...
            block_similarity[li] += torch.nn.functional.cosine_similarity(
                intermediate_outputs[ci][li - 1],
                intermediate_outputs[ci][li],
                dim=0,
            ).item()

    # average
    block_influence = [
        1 - bi / len(intermediate_outputs) for bi in block_similarity
    ]
    ...
    for _ in range(num_prune):
        ...
        for to_prune in range(num_blocks):
            ...
            ppl = evaluate_perplexity(
                model, calibration_sample, seq_len=2048, enable_tqdm=False
            )
...
def two_stage_2ssp(
    ...
    for sample in tqdm(
        calibration_dataset, total=len(calibration_dataset), desc="First stage"
    ):
    ...
    for li in range(num_blocks):
        ...
        logging.debug(
            f"Pruned MLP layer [{li}/{num_blocks}] - Mask: {top_indices}"
        )
