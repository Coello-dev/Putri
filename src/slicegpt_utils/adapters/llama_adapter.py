...
class LlamaModelAdapter(ModelAdapter):
    ...
    def get_layers(self) -> list[LayerAdapter]:
        return [
            self.layer_adapter_type(layer) for layer in self.model.model.layers
        ]
    ...
    def _from_pretrained(
        ...
        *,
        ...
        if not (
            ...
            # if not (
            #     model_name.startswith("meta-llama/Llama")
            #     or model_name.startswith("meta-llama/Meta-Llama")
            # ): # My stuff, trying to fix it
            return None
